...
interface GlobalVoiceButtonProps {
  ...
  /** Context for the AI: "cancer" (default), "agenda", "dossier" */
  context?: string;
}

export default function GlobalVoiceButton({ currentForm, onFieldsExtracted, context = 'cancer' }: GlobalVoiceButtonProps) {
  ...
  const processTranscript = useCallback(async (transcript: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('parse-voice-fields', {
        body: { transcript, currentForm, context },
      });
  ...
  }, [currentForm, onFieldsExtracted, context]);
          ...
          >
            {processing ? (
              ...
              <>
                ...
                {interim ? (
                  <p className="text-xs text-muted-foreground italic leading-relaxed line-clamp-3">« {interim} »</p>
                ) : (
