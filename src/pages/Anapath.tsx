...
export default function Anapath() {
  ...
  const openCase = (c: PendingCase, mode: 'edit' | 'view' = 'edit') => {
    setSelectedCase(c);
    setViewMode(mode);
    setForm({
                ...
                {filtered(completed).map(c => (
                  <div key={c.id} className="stat-card !py-3">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <Button size="sm" variant="outline" onClick={() => openCase(c, 'view')} className="shrink-0">
                        <Eye size={14} className="mr-1" /> Consulter
                      </Button>
                    </div>
                    {c.resultat_anapath && (
            ...
            {selectedCase && viewMode === 'view' ? (
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-muted/20 rounded-lg p-3">
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Référence</p>
                      <p className="text-sm font-medium font-mono">{selectedCase.ref_anapath || '—'}</p>
                    </div>
                    <div className="bg-muted/20 rounded-lg p-3">
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Date Analyse</p>
                      <p className="text-sm font-medium">{selectedCase.date_anapath ? new Date(selectedCase.date_anapath).toLocaleDateString('fr-DZ') : '—'}</p>
                    </div>
                    <div className="bg-muted/20 rounded-lg p-3">
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Médecin Pathologiste</p>
                      <p className="text-sm font-medium">{selectedCase.medecin_anapath || '—'}</p>
                    </div>
                    <div className="bg-muted/20 rounded-lg p-3">
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Code ICD-O</p>
                      <p className="text-sm font-medium font-mono">{selectedCase.code_icdo || '—'}</p>
                    </div>
                  </div>

                  <div className="bg-muted/20 rounded-lg p-4">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2">Résultat Histologique</p>
                    <p className="text-sm whitespace-pre-wrap leading-relaxed">{selectedCase.resultat_anapath}</p>
                  </div>
                </div>

                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setViewMode('edit')} className="flex-1">
                    <Microscope size={14} className="mr-1" /> Modifier
                  </Button>
              ...
              <div className="space-y-4">
                ...
                <div className="border border-dashed border-border rounded-lg p-4 text-center">
                  ...
                  <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="ocr-upload" onChange={e => e.target.files?.[0] && handleOCR(e.target.files[0])} />
                ...
                <div>
                  ...
                  <Textarea value={form.resultat_anapath} onChange={e => setForm(f => ({ ...f, resultat_anapath: e.target.value }))} rows={6} placeholder="Description macroscopique, microscopique, diagnostic histologique, grade..." className="mt-1" />
                </div>
