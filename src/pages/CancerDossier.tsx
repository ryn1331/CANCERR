...
import {
  ...
  FlaskConical, Dna, Target, Layers, ClipboardList, Droplets,
} from 'lucide-react';
...
interface CaseInfo {
  ...
  poids_kg: number | null; taille_cm: number | null; imc: number | null;
  alimentation: string | null; activite_physique: string | null;
  expositions_professionnelles: string | null; antecedents_familiaux: string | null;
  antecedents_familiaux_details: string | null; comorbidites: string[] | null;
  ...
  created_at: string;
  is_hemopathie: boolean; diagnostic_hemato: string | null; diagnostic_hemato_code: string | null;
...
export default function CancerDossier() {
  ...
  const [effetsRef, setEffetsRef] = useState<{ id: string; libelle: string }[]>([]);
  const [antecedentsRef, setAntecedentsRef] = useState<{ id: string; libelle: string }[]>([]);
  const [selectedEffets, setSelectedEffets] = useState<string[]>([]);
    ...
    if (showRechute) {
      setRechForm(f => {
        ...
        for (const [key, value] of Object.entries(fields)) {
          if (key in updated && value && !(updated as any)[key]) {
            (updated as any)[key] = value;
          }
        }
        return updated;
      });
    } else if (showTraitement) {
      setTraitForm(f => {
        const updated = { ...f };
        for (const [key, value] of Object.entries(fields)) {
          if (key in updated && value && !(updated as any)[key]) {
  ...
  const currentVoiceForm = showRechute ? rechForm : traitForm;
  ...
  useEffect(() => {
    const fetchRefs = async () => {
      const [effRes, antRes] = await Promise.all([
        supabase.from('effets_indesirables').select('id, libelle').eq('actif', true).order('sort_order'),
        supabase.from('antecedents_ref').select('id, libelle').eq('actif', true).order('sort_order'),
      ]);
      setEffetsRef((effRes.data as any[]) || []);
      setAntecedentsRef((antRes.data as any[]) || []);
    };
    fetchRefs();
  }, []);
  ...
  const addTraitement = async () => {
    if (!id || !user) return;
    setSaving(true);
    const effetsStr = selectedEffets.length > 0 ? selectedEffets.join(', ') : null;
    const antecedentsStr = selectedAntecedents.length > 0 ? selectedAntecedents.join(', ') : null;
    const notesVal = [antecedentsStr ? `Antécédents: ${antecedentsStr}` : null, traitForm.notes || null].filter(Boolean).join('\n') || null;
    const { error } = await supabase.from('traitements').insert({
      ...
      efficacite: traitForm.efficacite || null, effets_secondaires: effetsStr,
      medecin_traitant: traitForm.medecin_traitant || null, notes: notesVal, created_by: user.id,
    });
    if (error) toast.error(error.message);
    else { toast.success('Traitement ajouté'); setShowTraitement(false); setSelectedEffets([]); setSelectedAntecedents([]); fetchData(); }
    setSaving(false);
  };
  ...
  return (
    <AppLayout>
        ...
        <Tabs defaultValue="resume" className="space-y-4">
          ...
          <TabsContent value="resume">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Epidemio & Mode de vie */}
              <Card>
                <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><TrendingUp size={16} className="text-accent" /> Épidémiologie & Mode de vie</CardTitle></CardHeader>
                <CardContent className="space-y-2 text-sm">
                  ...
                  <Row label="Activité physique" value={caseInfo.activite_physique || '—'} />
                  {caseInfo.poids_kg && <Row label="Poids" value={`${caseInfo.poids_kg} kg`} />}
                  {caseInfo.taille_cm && <Row label="Taille" value={`${caseInfo.taille_cm} cm`} />}
                  {caseInfo.imc && <Row label="IMC" value={String(caseInfo.imc)} />}
                  {caseInfo.alimentation && <Row label="Alimentation" value={caseInfo.alimentation} />}
                  {caseInfo.expositions_professionnelles && <Row label="Expositions pro." value={caseInfo.expositions_professionnelles} />}
                  <Row label="Antécédents familiaux" value={caseInfo.antecedents_familiaux || '—'} />
                  {caseInfo.antecedents_familiaux_details && <Row label="Détails ATCD" value={caseInfo.antecedents_familiaux_details} />}
                  {caseInfo.comorbidites && caseInfo.comorbidites.length > 0 && <Row label="Comorbidités" value={caseInfo.comorbidites.join(', ')} />}
                  {caseInfo.symptomes && <Row label="Symptômes" value={caseInfo.symptomes} />}
                </CardContent>
              </Card>

              {/* Hémopathie */}
              {caseInfo.is_hemopathie && (
                <Card className="border-purple-300 bg-purple-50/50 dark:border-purple-700 dark:bg-purple-950/20">
                  <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><Droplets size={16} className="text-purple-600" /> Diagnostic Hématologique</CardTitle></CardHeader>
                  <CardContent className="space-y-2 text-sm">
                    <Row label="Diagnostic hémato" value={caseInfo.diagnostic_hemato || '—'} />
                    {caseInfo.diagnostic_hemato_code && <Row label="Code" value={caseInfo.diagnostic_hemato_code} mono />}
                    {caseInfo.examens_complementaires && <Row label="Examens complémentaires" value={caseInfo.examens_complementaires} />}
                  </CardContent>
                </Card>
              )}
                ...
                <DialogContent>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1.5 p-2 border rounded-md min-h-[40px] max-h-32 overflow-y-auto">
                        {effetsRef.length === 0 && <span className="text-xs text-muted-foreground">Aucun effet configuré</span>}
                        {effetsRef.map(e => (
                          <Badge
                            key={e.id}
                            variant={selectedEffets.includes(e.libelle) ? 'default' : 'outline'}
                            className={cn('cursor-pointer text-xs transition-colors', selectedEffets.includes(e.libelle) && 'bg-destructive text-destructive-foreground hover:bg-destructive/80')}
                            onClick={() => setSelectedEffets(prev => prev.includes(e.libelle) ? prev.filter(x => x !== e.libelle) : [...prev, e.libelle])}
                          >
                            {e.libelle}
                          </Badge>
                        ))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1.5 p-2 border rounded-md min-h-[40px] max-h-32 overflow-y-auto">
                        {antecedentsRef.length === 0 && <span className="text-xs text-muted-foreground">Aucun antécédent configuré</span>}
                        {antecedentsRef.map(a => (
                          <Badge
                            key={a.id}
                            variant={selectedAntecedents.includes(a.libelle) ? 'default' : 'outline'}
                            className={cn('cursor-pointer text-xs transition-colors', selectedAntecedents.includes(a.libelle) && 'bg-purple-600 text-white hover:bg-purple-500')}
                            onClick={() => setSelectedAntecedents(prev => prev.includes(a.libelle) ? prev.filter(x => x !== a.libelle) : [...prev, a.libelle])}
                          >
                            {a.libelle}
                          </Badge>
                        ))}
            ...
            {traitements.length === 0 ? (
              ...
              <Card key={t.id}>
                <CardContent className="p-4">
                  ...
                  {t.effets_secondaires && (
                    <div className="mt-1">
                      <span className="text-sm font-medium">Effets indésirables:</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {t.effets_secondaires.split(', ').map((e, i) => (
                          <Badge key={i} variant="outline" className="text-xs text-destructive border-destructive/30">{e}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
                  {t.notes && t.notes.includes('Antécédents:') && (
                    <div className="mt-1">
                      <span className="text-sm font-medium">Antécédents:</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {t.notes.split('\n').filter(l => l.startsWith('Antécédents:')).map(l => l.replace('Antécédents: ', '')).join(', ').split(', ').map((a, i) => (
                          <Badge key={i} variant="outline" className="text-xs text-purple-600 border-purple-300">{a}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
      ...
      {(showRechute || showTraitement) && (
        <GlobalVoiceButton currentForm={currentVoiceForm as any} onFieldsExtracted={handleVoiceFields} context="dossier" />
      )}
    </AppLayout>
  );
}
